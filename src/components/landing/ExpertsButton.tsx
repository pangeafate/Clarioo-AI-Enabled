/**
 * ExpertsButton Component
 *
 * @purpose Button to open the Experts modal
 *
 * FEATURES:
 * - Same size as Projects button (ViewToggleButton)
 * - Violet gradient outline (from-purple-600 to-blue-600)
 * - Fill color matching Clarioo badge
 * - Gradient text color
 * - Opens ExpertsModal on click
 *
 * DESIGN SPECS:
 ...
 * - Typography: Same as Projects button (TYPOGRAPHY.button.small)
 */
...
interface ExpertsButtonProps {
  onClick: () => void;
}

export const ExpertsButton = ({ onClick }: ExpertsButtonProps) => {
  return (
    <motion.button
      onClick={onClick}
      ...
      aria-label="Open Experts information"
    >
      ...
      <div className="relative flex items-center gap-1 xs:gap-2">
        ...
        <Users className="w-3 h-3 xs:w-4 xs:h-4 sm:w-5 sm:h-5 text-purple-600" />
      </div>
    </motion.button>
  );
};
